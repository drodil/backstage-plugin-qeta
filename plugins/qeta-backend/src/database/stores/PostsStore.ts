    ...
    const rows = await this.db.transaction(async trx => {
      ...
      if (
        ...
        const hasContentChanges =
          ...
          (content !== undefined && content !== currentPost.content) ||
          (url !== undefined && url !== currentPost.url) ||
          (headerImage !== undefined &&
            headerImage !== currentPost.headerImage);

        let hasTagOrEntityChanges = false;
        if (
          !hasContentChanges &&
          (tags !== undefined || entities !== undefined)
        ) {
          const currentTags = await this.getCurrentTags(id);
          const currentEntities = await this.getCurrentEntities(id);
          if (tags !== undefined) {
            const sortedNew = [...(filterTags(tags) ?? [])].sort();
            const sortedOld = [...currentTags].sort();
            hasTagOrEntityChanges =
              sortedNew.length !== sortedOld.length ||
              sortedNew.some((t, i) => t !== sortedOld[i]);
          }
          if (!hasTagOrEntityChanges && entities !== undefined) {
            const sortedNew = [...entities].sort();
            const sortedOld = [...currentEntities].sort();
            hasTagOrEntityChanges =
              sortedNew.length !== sortedOld.length ||
              sortedNew.some((e, i) => e !== sortedOld[i]);
          }
        }

        if (hasContentChanges || hasTagOrEntityChanges) {
